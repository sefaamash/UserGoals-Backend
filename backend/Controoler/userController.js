...
const logInUser = asyncHandler(async (req, res) => {
    ...
    //Check User Password
    if(user && (await bcrypt.compare(password,user.password))) //Now we compare the password typed by user and and original password which is not hashed one if its match then proceed
    {
        res.json({
            _id: user.id,
            name: user.name,
